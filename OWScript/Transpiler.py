...
class Transpiler:
    ...
    def visitIf(self, node, scope):
        ...
        skip_false = []
        true_code = self.visit_children(node.true_block, scope)
        false_code = []
        if node.false_block:
            skip_false.append(self.tabs + 'Skip({});\n')
            if type(node.false_block) == If:
                false_code.extend(self.visit(node.false_block, scope))
            else:
                for line in node.false_block.children:
                    false_code.append(self.tabs + self.visit(line, scope) + ';\n')
        #skip_code = skip_code.format(cond, true_code.count(';\n') + bool(node.false_block))
        if false_code:
            pass#skip_false = skip_false.format(false_code.count(';\n') + bool(type(node.false_block) == If))
        lines = [skip_code] + true_code + skip_false + false_code
        return lines
    ...
    def visitFor(self, node, scope):
        ...
        print('lines:', scope.get('ivar_ruleblock').value)
