...
def transpile(text, path, args):
    ...
    transpiler = Transpiler(tree=tree, path=path, logger=logger, credit=args.credit)
    code = transpiler.run()
    if args.min:
        code = re.sub(r'[\s\n]*', '', code)
    if not args.save:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--credit', type=int, default=1, help='Display the author credit rule in the output (0=Hide, 1=Show)')
